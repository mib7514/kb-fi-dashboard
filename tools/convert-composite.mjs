...
// 루트에 xlsx 여럿(만기확장·구 composite·ktb3y 등) → '만기확장' 우선, 없으면 spread 시트 보유 첫 파일.
const xlsxs = readdirSync(ROOT).filter(f => f.endsWith('.xlsx') && !f.startsWith('~$'));
const xlsxName = xlsxs.find(f => f.includes('만기확장')) || xlsxs.find(f => f.includes('composite')) || xlsxs[0];
if (!xlsxName) { console.error('❌ 레포 루트에 .xlsx 파일이 없습니다.'); process.exit(1); }
console.log(`소스: ${xlsxName}`);
